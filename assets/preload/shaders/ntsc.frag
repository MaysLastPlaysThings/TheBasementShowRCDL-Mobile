...
vec4 pass1(vec2 uv)
{
	...
	#elif defined(THREE_PHASE)
		float chroma_phase = 0.67 * PI * (mod(fragCoord.y, 3.0) + float(uFrame));
	#endif
...
vec4 fetch_offset(vec2 uv, float offset, float one_x) {
	return pass1(uv + vec2((offset - 2.5) * one_x, 0.0)).xyzw;
}
...
void main()
{
	...
	for (int i = 0; i < TAPS; i++)
	{
		...
		signal += sums * vec4(luma_filter[i], chroma_filter[i], chroma_filter[i], 4.5);
	}
