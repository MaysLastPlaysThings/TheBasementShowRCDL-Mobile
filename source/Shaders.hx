...
{
    vec2 look = uv;
    float window = 1.0 / (1.0 + 20.0 * (look.y - mod(iTime / 4.0, 1.0)) * (look.y - mod(iTime / 4.0, 15.0)));
    look.x = look.x + (sin(look.y * 10.0 + iTime) / 540.0 * onOff(4.0, 4.0, 0.3) * (1.0 + cos(iTime * 80.0)) * window) * (glitchModifier * 2.0);
    float vShift = 0.4 * onOff(2.0, 3.0, 0.9) * (sin(iTime) * sin(iTime * 20.0) + (1.0 + 0.1 * sin(iTime * 200.0) * cos(iTime)));
...
vec2 scandistort(vec2 uv)
{
    ...
    uv.x -= 0.015 * mix(texture2D(noiseTex, vec2(uv.x, amount)).r * amount, amount, 0.2);
    return uv;
}
...
void main()
{
    ...
    video.r = getVideo(vec2(x + uv.x + 0.001, uv.y + 1.0)).x + abs(sin(0.12 * redOpac));
    ...
    vigAmt = 2.0 + 0.1 * sin(iTime + 5.0 * cos(iTime * 5.0));

    float vignette = (1.1 - vigAmt * (uv.y - 0.5) * (uv.y - 0.5)) * (0.1 - vigAmt * (uv.x - 0.5) * (uv.x - 0.5));
...
class MenusVCRShader extends FlxShader
{
  @:glFragmentSource('
