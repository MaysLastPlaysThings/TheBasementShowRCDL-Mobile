...
class OldTVShader extends FlxShader
{
    ...
    vec4 getVideo(vec2 uv)
      {
      	vec2 look = uv;
        	float window = 1.0/(1.+20.0 * (look.y - mod(iTime/4.0, 1.0)) * (look.y - mod(iTime/4.0, 15.0)));
        	look.x = look.x + (sin(look.y * 10.0 + iTime)/540.0 * onOff(4.0, 4.0,0.3) * (1.0 + cos(iTime * 80.0)) * window) * (glitchModifier * 2.0);
        	float vShift = 0.4 * onOff(2.0, 3.0, 0.9) * (sin(iTime) * sin(iTime*20.) +
        										 (1 + 0.1 * sin(iTime * 200.0) * cos(iTime)));
