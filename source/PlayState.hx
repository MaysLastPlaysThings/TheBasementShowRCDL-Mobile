...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Goodbye.', 0.1], //From 0% to 19%
		['LET\'S GREET', 0.2], //From 0% to 19%
		['STOP', 0.3], //From 0% to 19%
		['NO', 0.4], //From 20% to 39%
		['Pain in the neck', 0.5], //From 40% to 49%
		['Outrageous', 0.6], //From 50% to 59%
		['Rough', 0.69], //From 60% to 68%
		['Try Harder', 0.7], //69%
		['I Expect Better', 0.89], //From 70% to 79%
		['Not Bad', 0.99], //From 80% to 89%
	...
	var stopSnowThing:Bool = false;

	public var enableDaEvent:Bool = false;
	...
	public var snow1:FlxSprite;
	public var snow2:FlxSprite;
	public var snow3:FlxSprite;
	...
	public var enableJump:Bool = false;
	...
	private var oldhealthBarBG:AttachedSprite;
	...
	override public function create()
	{
		...
		tvShader = new Shaders.OldTVShader();
		...
		if (PlayState.SONG.song == 'House for Sale OLD' || 
			...
			Lib.application.window.title = 'The Basement Show - Now Playing: ' + PlayState.SONG.song + ' - By: MarStarBro, Juno, Punkett';
		}
		else if (PlayState.SONG.song == 'Invade OLD' || PlayState.SONG.song == 'Black Humor OLD') {
			Lib.application.window.title = 'The Basement Show - Now Playing: ' + PlayState.SONG.song + ' - By: CZTV-28';
		}
		else {
		...
		jerryScream = new FlxSprite(0, 0).loadGraphic(Paths.image('JS_Jerry', 'shared'));
		jerryScream.screenCenter();
		jerryScream.cameras = [camOther];
		jerryScream.alpha = 0.00001;

		snow1 = new FlxSprite(0, 0).loadGraphic(Paths.image('snow', 'shared'));
		snow1.screenCenter();
		snow1.scale.set(1.2, 1.2);
		snow1.cameras = [camOther];
		snow2 = new FlxSprite(0, 0).loadGraphic(Paths.image('snow2', 'shared'));
		snow2.screenCenter();
		snow2.scale.set(1.2, 1.2);
		snow2.cameras = [camOther];
		snow3 = new FlxSprite(0, 0).loadGraphic(Paths.image('snow3', 'shared'));
		snow3.screenCenter();
		snow3.scale.set(1.2, 1.2);
		snow3.cameras = [camOther];
	...
	for (folder in foldersToCheck) {
		for (asset in Assets.list()) {
			if (asset.endsWith(".lua") && !filesPushed.contains(asset) && asset.contains(folder)) {
				luaArray.push(new FunkinLua(asset));
				trace("loaded script:" + asset);
				filesPushed.push(asset);
			}
		}
	}
	#end
		...
		HB = new FlxSprite().loadGraphic(Paths.image('healthbarHQ', 'shared'));
		HB.scrollFactor.set();
		HB.screenCenter(X);
		HB.cameras = [camHUD];
		HB.scale.set(0.418, 0.4);
		HB.visible = !ClientPrefs.hideHud;
		...
		if (TBSFreeplayState.chosendifficulty == 'hard') {
		...
		oldhealthBarBG.scrollFactor.set();
		oldhealthBarBG.visible = !ClientPrefs.hideHud;
		oldhealthBarBG.xAdd = -4;
		healthBar.scale.y = 0.5001;
		oldhealthBarBG.cameras = [camHUD];
		oldhealthBarBG.sprTracker = healthBar;
		oldhealthBarBG.yAdd = 5;
		HB.alpha = 0;
		add(oldhealthBarBG);
		if(ClientPrefs.downScroll) oldhealthBarBG.y = 0.11 * FlxG.height;
		}
		...
		if (TBSFreeplayState.chosendifficulty == 'old' || TBSFreeplayState.chosendifficulty == 'older') {
			ratingStuff = [
		...
		else if (PlayState.SONG.song == 'Come for Revenge' || PlayState.SONG.song == 'Reburning' || PlayState.SONG.song == 'Paralyzed') {
			ratingStuff = [
		...
		else if (PlayState.SONG.song == 'Mucho Mouse') {
			ratingStuff = [
				['F--', 0.2], //From 0% to 19%
				['F', 0.4], //From 40% to 49%
				['E', 0.6], //From 50% to 59%
		...
		else if (PlayState.SONG.song == 'Meme Mania') {
			ratingStuff = [
		...
		else if (PlayState.SONG.song == 'Faded') {
			ratingStuff = [
				['Dead', 0.2], //From 0% to 19%
				['Lost', 0.4], //From 40% to 49%
				['Danger', 0.6], //From 50% to 59%
				['Sussy', 0.7], //69%
				['Sus', 0.8], //From 70% to 79%
				['Hmmmmm', 0.9], //From 80% to 89%
				['Good', 1], //From 90% to 99%
				['Greatest Crewmate', 1] //The value on this one isn't used actually, since Perfect is always "1"
			];
		}
		else {
			ratingStuff = [
		...
		scoreTxt = new FlxText(0, healthBarBG.y + 46, 1250, "", 20);
	...
	if (PlayState.SONG.song == 'house-for-sale' ||
		...
		if (ClientPrefs.judjmentbool) judjTxt.cameras = [camHUD];
		...
		if (PlayState.SONG.song == 'house-for-sale' ||
			...
			camHor.setFilters([new ShaderFilter(tvShader)]);
			}
		}
	...
	public var countdownReadyy:FlxSprite;
	...
	public function startCountdown():Void
	{
		...
		for (note in unspawnNotes) {
			if (note.mustPress && !note.isSustainNote && !note.ignoreNote && !note.hitCausesMiss)
				noteCount += 1;
		}
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				{
					case 0:
						countdownReadyy = new FlxSprite().loadGraphic(Paths.image(introAlts[0]));
						countdownReadyy.cameras = [camHUD];
						countdownReadyy.scrollFactor.set();
						countdownReadyy.scale.set(1.3, 1.3);
						countdownReadyy.updateHitbox();

						if (PlayState.isPixelStage)
							countdownReadyy.setGraphicSize(Std.int(countdownReady.width * daPixelZoom));

						countdownReadyy.screenCenter();
						countdownReadyy.antialiasing = antialias;
						insert(members.indexOf(notes), countdownReadyy);
						FlxTween.tween(countdownReadyy.scale, {y: 1.45, x: 1.45}, 0.3, {ease: FlxEase.quadOut, type: FlxTweenType.BACKWARD});
						FlxG.sound.play(Paths.sound('count'), 0.6);
					case 1:
						remove(countdownReadyy);
						countdownReadyy.destroy();
						countdownReady = new FlxSprite().loadGraphic(Paths.image(introAlts[1]));
						countdownReady.cameras = [camHUD];
						countdownReady.scrollFactor.set();
						countdownReady.scale.set(1.3, 1.3);
						...
						FlxTween.tween(countdownReady.scale, {y: 1.45, x: 1.45}, 0.3, {ease: FlxEase.quadOut, type: FlxTweenType.BACKWARD});
						FlxG.sound.play(Paths.sound('count'), 0.6);
					case 2:
						...
						countdownSet.scale.set(1.3, 1.3);
						...
						FlxTween.tween(countdownSet.scale, {y: 1.45, x: 1.45}, 0.3, {ease: FlxEase.quadOut, type: FlxTweenType.BACKWARD});
						FlxG.sound.play(Paths.sound('count'), 0.6);
					case 3:
						remove(countdownSet);
						...
						FlxTween.tween(countdownGo.scale, {y: 1.45, x: 1.45}, 0.3, {ease: FlxEase.quadOut, type: FlxTweenType.BACKWARD});
						FlxG.sound.play(Paths.sound('finish'), 0.6);
					case 4:
						remove(countdownGo);
						countdownGo.destroy();
				}
				...
				switch (swagCounter)
				{
					case 0:
						...
						countdownReadyy.antialiasing = antialias;
						insert(members.indexOf(notes), countdownReadyy);
						FlxTween.tween(countdownReadyy, {/*y: countdownReady.y + 100,*/ alpha: 0}, Conductor.crochet / 1000, {
	...
	if (TBSFreeplayState.chosendifficulty == 'hard') {
		...
		scoreTxt.text = 'Score: ' + songScore + ' / Combo Breaks: ' + songMisses + ' / Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '% [' + ratingName + ']';
		}
		else {
		...
		scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Ratings: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 0) + '%)';
		}
		...
		scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Ratings: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%) - ' + ratingFC;
		}
	}

	if (TBSFreeplayState.chosendifficulty == 'hard') {
		if (ClientPrefs.judjmentbool) {
		judjTxt.text = 'Hits: ' + hitsCount + ' - ' + noteCount + '\nCombo: ' + combo +' (Max: '+ maxCombo + ')\nWellDone: ' + sicks + '\nOk: ' + goods + '\nNoob:' + bads + '\nSuck: ' + shits;
		}
	}
	else {
		if (ClientPrefs.judjmentbool) {
		judjTxt.text = 'Max Combo: '+ maxCombo + '\nCombo : ' + combo +'\nWellDone: ' + sicks + '\nOk: ' + goods + '\nNoob:' + bads + '\nSuck: ' + shits;
		}
	}
	...
	function startSong():Void
	{
		...
		if (PlayState.SONG.song == 'Steep Slopes') {
			songLength = 122727;
		}
		else if (PlayState.SONG.song == 'Faded') {
		    songLength = 138000;
		...
		if (PlayState.SONG.song == 'House for Sale OLD' || 
			...
			PlayState.SONG.song == 'Invade OLDER' ||
			PlayState.SONG.song == 'Funny Cartoon OLD') {
			DiscordClient.changePresence(detailsText, SONG.song + ' - By: JerryWannaRat', iconP2.getCharacter(), true, songLength);
		}
		else if (PlayState.SONG.song == 'Invade OLD' || PlayState.SONG.song == 'Black Humor OLD') {
		DiscordClient.changePresence(detailsText, SONG.song + ' - By: CZTV-28', iconP2.getCharacter(), true, songLength);
		}
		else {
		DiscordClient.changePresence(detailsText, SONG.song + ' - By: ' + TBSFreeplayState.songCredits[TBSFreeplayState.curSelected], iconP2.getCharacter(), true, songLength);
		}
		#end
		...
		if (paused)
		{
			...
			if (startTimer != null && startTimer.finished)
			{
				if (PlayState.SONG.song == 'House for Sale OLD' || 
					...
					PlayState.SONG.song == 'Invade OLDER' ||
					PlayState.SONG.song == 'Funny Cartoon OLD') {
					DiscordClient.changePresence(detailsText, SONG.song + ' - By: JerryWannaRat', iconP2.getCharacter(), true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
				}
				else if (PlayState.SONG.song == 'Invade OLD' || PlayState.SONG.song == 'Black Humor OLD') {
					DiscordClient.changePresence(detailsText, SONG.song + ' - By: CZTV-28', iconP2.getCharacter(), true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
				}
				else {
				DiscordClient.changePresence(detailsText, SONG.song + ' - By: ' + TBSFreeplayState.songCredits[TBSFreeplayState.curSelected], iconP2.getCharacter(), true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
				}
			}
			else
			{
				...
				DiscordClient.changePresence(detailsText, SONG.song + ' - By: ' + TBSFreeplayState.songCredits[TBSFreeplayState.curSelected], iconP2.getCharacter());
				}
			}
			#end
		}
		...
		if (health > 0 && !paused)
		{
			...
			else
			{
				if (PlayState.SONG.song == 'House for Sale OLD' || 
					...
					PlayState.SONG.song == 'Invade OLDER' ||
					PlayState.SONG.song == 'Funny Cartoon OLD') {
					DiscordClient.changePresence(detailsText, SONG.song + ' - By: JerryWannaRat', iconP2.getCharacter());
				}
				else if (PlayState.SONG.song == 'Invade OLD' || PlayState.SONG.song == 'Black Humor OLD') {
					DiscordClient.changePresence(detailsText, SONG.song + ' - By: CZTV-28', iconP2.getCharacter());
				}
				else {
				DiscordClient.changePresence(detailsText, SONG.song + ' - By: ' + TBSFreeplayState.songCredits[TBSFreeplayState.curSelected], iconP2.getCharacter());
				}
	...
	override public function onFocusLost():Void
	{
		...
		if (health > 0 && !paused)
		{
			if (PlayState.SONG.song == 'House for Sale OLD' || 
				...
				PlayState.SONG.song == 'Invade OLDER' ||
				PlayState.SONG.song == 'Funny Cartoon OLD') {
					DiscordClient.changePresence(detailsPausedText, SONG.song + ' - By: JerryWannaRat', iconP2.getCharacter());
			}
			else if (PlayState.SONG.song == 'Invade OLD' || PlayState.SONG.song == 'Black Humor OLD') {
				DiscordClient.changePresence(detailsPausedText, SONG.song + ' - By: CZTV-28', iconP2.getCharacter());
			}
			else {
			DiscordClient.changePresence(detailsPausedText, SONG.song + ' - By: ' + TBSFreeplayState.songCredits[TBSFreeplayState.curSelected], iconP2.getCharacter());
			}
		}
		#end
...
//			Lib.application.window.title = "The Basement Show";
		...
		if (stopSnowThing && (snow1.alpha != 0 || snow2.alpha != 0 || snow3.alpha != 0)) {
			snow1.alpha = FlxMath.lerp(snow1.alpha, 0, CoolUtil.boundTo(elapsed * 11.8, 0, 1));
			snow2.alpha = FlxMath.lerp(snow2.alpha, 0, CoolUtil.boundTo(elapsed * 11.8, 0, 1));
			snow3.alpha = FlxMath.lerp(snow3.alpha, 0, CoolUtil.boundTo(elapsed * 11.8, 0, 1));
		}
		...
		var mult:Float = FlxMath.lerp(1, iconP1.scale.x, CoolUtil.boundTo(1 - (elapsed * timieBouncy * playbackRate), 0, 1));
		iconP1.scale.set(mult, mult);

		var mult:Float = FlxMath.lerp(1, iconP2.scale.x, CoolUtil.boundTo(1 - (elapsed * timieBouncy * playbackRate), 0, 1));
		...
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				if (PlayState.SONG.song == 'House for Sale OLD' || 
					...
					PlayState.SONG.song == 'Invade OLDER' ||
					PlayState.SONG.song == 'Funny Cartoon OLD') {
					DiscordClient.changePresence("Game Over - " + detailsText, SONG.song  + ' - By: JerryWannaRat', iconP2.getCharacter());
				}
				else if (PlayState.SONG.song == 'Invade OLD' || PlayState.SONG.song == 'Black Humor OLD') {
				DiscordClient.changePresence("Game Over - " + detailsText, SONG.song  + ' - By: CZTV-28', iconP2.getCharacter());
				}
				else {
				DiscordClient.changePresence("Game Over - " + detailsText, SONG.song  + ' - By: ' + TBSFreeplayState.songCredits[TBSFreeplayState.curSelected], iconP2.getCharacter());
				}
				#end
				isDead = true;
				return true;
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (daNote.noteType == 'Orange Bone Note' && ClientPrefs.mechanicsOn)
			{
				health -= 0.5;
			}
	...
	{
		if (note.noteType == 'knife' && !cpuControlled)
		{
			kys();
		}
		if (!note.wasGoodHit)
		{
			...
			if (note.noteType == 'Blue Bone Note') {
			health -= 0.5;
			}

			if (note.noteType == 'Orange Bone Note' && ClientPrefs.mechanicsOn) {
			health += 0.12;
			}

			if (note.noteType == 'snow') {
				stopSnowThing = false;
				snowLayer += 1;
	...
	public function spawnNoteSplashOnNote(note:Note) {
		if(ClientPrefs.slpashAlpha != 0 && note != null) {
	...
	function kys() {
	FlxG.sound.play(Paths.sound('jerry'));
	jerryScream.alpha = 1;
	add(jerryScream);
	jerryScream.screenCenter();
	new FlxTimer().start(0.5, function(tmr:FlxTimer) {
		FlxTween.tween(jerryScream, {alpha: 0}, 0.5, {ease: FlxEase.circOut});
	});
	new FlxTimer().start(0.52, function(tmr:FlxTimer) {
	remove(jerryScream);
	});
	}
	...
	override function beatHit()
	{
		...
		iconP1.scale.set(1.26, 1.26);
		iconP2.scale.set(1.26, 1.26);
	...
	override function sectionHit()
	{
		...
		if (PlayState.SONG.song == 'None of All' && curSection > 168) {
			Conductor.changeBPM(100);
		}
