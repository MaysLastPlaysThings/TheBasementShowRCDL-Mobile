...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		modBanner = new FlxSprite(0, 0).loadGraphic(Paths.image('titlescreen/modBanner'));
		add(modBanner);
		modBanner.scrollFactor.set();
		modBanner.antialiasing = true;
		modBanner.screenCenter();

		modLogo = new FlxSprite(255, 300).loadGraphic(Paths.image('titlescreen/TBSLogoBlank'));
		add(modLogo);
	...
	function addMoreText(text:String, ?offset:Float = 0)
	{
		if(textGroup != null && credGroup != null) {
			...
			coolText.y += (textGroup.length * 75) + 200 + offset;
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 3:
					#if PSYCH_WATERMARKS
					addMoreText('The Basement Team');
					#else
					addMoreText('Presents');
					#end
				case 4:
					addMoreText('Geez So Many People');
				// credTextShit.text += '\npresent...';
